...
const MyJobAlerttem = ({ item }: any) => {
  return (
    <div className="px-2 py-4 border border-gray-200 hover:border-blue-500 rounded-[6px] mb-4 hover:shadow-lg transition-all">
      ...
      <p className="text-gray-500 text-[14px]">{item.descriptionText && item.descriptionText.length > 300 ? item.descriptionText.slice(0, 300) + " ..." : item.descriptionText}</p>
      <div className="border border-dashed border-r-0 border-l-0 border-b-0 mt-4 pt-2">
        <p className="underline flex justify-start items-center mb-4">
        ...
        <div className="flex justify-start items-center flex-wrap">
          ...
          {item.skills &&  item.skills .map((each: any, order: any) => <p key={order} className="border rounded-[4px] text-[12px] px-2 mr-1 mb-1">{each}</p>)}
        </div>
      </div>
    </div>
  );
