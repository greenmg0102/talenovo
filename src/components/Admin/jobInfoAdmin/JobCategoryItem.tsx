...
import { jobCategoryPut, jobCategoryDelete } from '@/store/action/admin/jobInfo/jobCategory'

const JobTypeCategoryItem = ({ index, item, setTyepList }: any) => {
  ...
  const deleteItem = async (id: any) => {
    setActive(!active)
    let result = await jobCategoryDelete({ id: id })
    if (result.result) {
  ...
  const saveItem = async (id: any) => {
    setActive(!active)
    let result = await jobCategoryPut({ id: id, category: value })
    if (result.result) {
...
export default JobTypeCategoryItem;
