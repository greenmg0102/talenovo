...
import { currencyPut, currencyDelete } from '@/store/action/admin/jobInfo/currency'

const CurrencyListItem = ({ index, item, setTyepList }: any) => {
  ...
  const changeItem = () => {
    setActive(!active)
    setValue(item.currency)
  }

  const deleteItem = async (id: any) => {
    setActive(!active)
    let result = await currencyDelete({ id: id })
    if (result.result) {
  ...
  const saveItem = async (id: any) => {
    setActive(!active)
    let result = await currencyPut({ id: id, currency: value })
    if (result.result) {
