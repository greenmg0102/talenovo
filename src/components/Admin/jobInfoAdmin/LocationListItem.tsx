...
import { jobLocationPut, jobLocationDelete } from '@/store/action/admin/jobInfo/jobLocation'

const LocationList = ({ index, item, setTyepList }: any) => {
  ...
  const deleteItem = async (id: any) => {
    setActive(!active)
    let result = await jobLocationDelete({ id: id })
    if (result.result) {
  ...
  const saveItem = async (id: any) => {
    setActive(!active)
    let result = await jobLocationPut({ id: id, location: value })
    if (result.result) {
