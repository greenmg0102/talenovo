...
import { currencyTypePut, currencyTypeDelete } from '@/store/action/admin/jobInfo/currencyType'

const CurrencyType = ({ index, item, setTyepList }: any) => {
  ...
  const changeItem = () => {
    setActive(!active)
    setValue(item.currencyType)
  }

  const deleteItem = async (id: any) => {
    setActive(!active)
    let result = await currencyTypeDelete({ id: id })
    if (result.result) {
  ...
  const saveItem = async (id: any) => {
    setActive(!active)
    let result = await currencyTypePut({ id: id, currencyType: value })
    if (result.result) {
    ...
    <div className="w-full mb-2 flex justify-between items-center p-1 border border-white hover:border-dashed hover:border-gray-200 transition-all">
      ...
      <div className="w-[calc(100%-40px)] flex justify-between items-center">
        {active ?
          ...
          :
          <p className=''>{item.currencyType}</p>
        }
