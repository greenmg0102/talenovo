...
import { jobTagPut, jobTagDelete } from '@/store/action/admin/jobInfo/jobTag'


const TagItem = ({ index, item, setTyepList }: any) => {
  ...
  const deleteItem = async (id: any) => {
    setActive(!active)
    let result = await jobTagDelete({ id: id })
    if (result.result) {
  ...
  const saveItem = async (id: any) => {
    setActive(!active)
    let result = await jobTagPut({ id: id, tag: value })
    if (result.result) {
