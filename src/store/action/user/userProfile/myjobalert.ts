...
export async function myJobAlert() {
    const res = await fetch('http://104.128.55.140:443/api/user/user-profile/my-job-alert', {
        method: 'GET',
    });
...
export async function myBookMarkJob() {
    const res = await fetch('http://104.128.55.140:443/api/user/user-profile/my-bookmark-job', {
        method: 'GET',
    });
    return await res.json();
}
