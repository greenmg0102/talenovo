...
export async function myJobAlert() {
    const res = await fetch('http://https://talenovo.com/api/user/user-profile/my-job-alert', {
        method: 'GET',
    });
...
export async function myBookMarkJob() {
    const res = await fetch('http://https://talenovo.com/api/user/user-profile/my-bookmark-job', {
        method: 'GET',
    });
    return await res.json();
}
